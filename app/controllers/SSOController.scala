...
import it.gov.***REMOVED***.common.sso.common.{CacheWrapper, CredentialManager, LoginInfo}
import it.gov.***REMOVED***.common.utils.Credentials
import it.gov.***REMOVED***.securitymanager.service.utilities.RequestContext.execInContext
import it.gov.***REMOVED***.securitymanager.service.utilities.{ConfigReader, Utils}
...
class SSOController @Inject()(ws: WSClient, config: ConfigurationProvider, cacheWrapper: CacheWrapper, loginClientLocal:LoginClientLocal) extends Controller {
  ...
  def biOpenLogin  = Action.async (parse.urlFormEncoded){ implicit request =>
    execInContext[Future[Result]] ("biOpenLogin"){ () =>
      ...
      /*
      val out = for{
        a <- ws.url("***REMOVED***://security-manager.default.svc.cluster.local:9000/security-manager/v1/token").withHeaders("Authorization" -> bearer).get
      ...
      out.map { cookies =>
        ...
        val tt = scala.util.Try {
          val biCookie = cookies.head.copy(domain = Option(".open.***REMOVED***.***REMOVED***.test"))
          val content = views.html.Application.bi_open_login(biCookie)
          Ok(content)
        }
