...
import it.gov.***REMOVED***.securitymanager.service.{WebHDFSApiClient, WebHDFSApiProxy}
import it.gov.***REMOVED***.securitymanager.service.utilities.RequestContext.execInContext
import play.api.Logger
import play.api.***REMOVED***.HttpEntity
  ...
  def retriveACL(path:String) = Action.async { implicit request =>
    execInContext[Future[Result]]("retriveACL") { () =>
      ...
      webHDFSApiClient.getAclStatus(path).map {
        case Right(r) => Ok(r)
        case Left(l) => new Status(l.***REMOVED***Code).apply(l.jsValue.toString())
      }
  ...
  def callWebHdfs(path:String) = Action.async (wehdfsBodyParser) { implicit request =>
    ...
    execInContext[Future[Result]]("callWebHdfs") { () =>
      ...
      webHDFSApiProxy.callHdfsService(request.method, path, queryString, None).flatMap {
        case Right(r) =>  if( r.***REMOVED***Code == 307 && r.locationHeader.nonEmpty)
        ...
        case Left(l) => Future.successful{ new Status(l.***REMOVED***Code).apply(l.jsValue.toString()) }
      }
  ...
  def callWebHdfs(path:String) = Action.async { implicit request =>
    execInContext[Future[Result]]("callWebHdfs") { () =>
      ...
      webHDFSApiProxy.callHdfsService(request.method, path, queryString).map {
        ...
        //if(r.***REMOVED***Code==307 && r.locationHeader.nonEmpty)
        //call
  ...
  def callWebHdfsAction2[A](action: Action[A], path:String)= Action.async(action.parser) { request =>
    ...
    execInContext[Future[Result]]("callWebHdfs2") { () =>
      ...
      webHDFSApiProxy.callHdfsService(request.method, path, queryString).flatMap {
        ...
        case Right(r) => println("request.method "+request.method);println("r.***REMOVED***Code "+r.***REMOVED***Code);println("r.locationHeader "+r.locationHeader);
          if (request.method == "PUT" && r.***REMOVED***Code == 307 && r.locationHeader.nonEmpty) {
        ...
        case Left(l) => println("request.method "+request.method);println("r.***REMOVED***Code "+l.***REMOVED***Code);println("r.locationHeader "+l.locationHeader);Future {
          new Status(l.***REMOVED***Code).apply(l.jsValue.toString())
        }
      }
    ...
    def callWebHdfsAction2[A](action: Action[A], path:String)= Action.async(action.parser) { request =>
      ...
      execInContext[Future[Result]]("callWebHdfs2") { () =>
        ...
        webHDFSApiProxy.callHdfsService(request.method, path, queryString).flatMap {
          ...
          case Right(r) => println("request.method "+request.method);println("r.***REMOVED***Code "+r.***REMOVED***Code);println("r.locationHeader "+r.locationHeader);if (request.method == "PUT" && r.***REMOVED***Code == 307 && r.locationHeader.nonEmpty) {
