package it.gov.***REMOVED***.sso

import java.net.URLEncoder

import cats.data.EitherT
import com.google.inject.{Inject, Singleton}
import it.gov.***REMOVED***.common.sso.common.{LoginInfo, Role, SecuredInvocationManager}
import it.gov.***REMOVED***.common.utils.WebServiceUtil
import it.gov.***REMOVED***.securitymanager.service.IntegrationService
import it.gov.***REMOVED***.securitymanager.service.utilities.{AppConstants, ConfigReader}
import play.api.libs.json._
import play.api.libs.ws.{WSAuthScheme, WSClient, WSResponse}
import security_manager.yaml.{DafGroupInfo, Error, GroupList, IpaGroup, IpaUser, Success}
import cats.implicits._
import it.gov.***REMOVED***.securitymanager.service
import it.gov.***REMOVED***.securitymanager.service.ProcessHandler._
...
class ApiClientIPA @Inject()(secInvokeManager:SecuredInvocationManager,loginClientLocal: LoginClientLocal,wsClient: WSClient){
  ...
  def testH:Future[Either[Error,Success]]={
    wsClient.url("***REMOVED***://master:50470/webhdfs/v1/app?op=GETFILESTATUS").withAuth("krbtgt/DAF.GOV.IT@DAF.GOV.IT","andreacherici@DAF.GOV.IT", WSAuthScheme.SPNEGO).get().map{ resp =>
      println("----->>>"+resp.body)
  ...
  def isWorkgroup(ipaGroup:IpaGroup) = {
    ...
    if( ipaGroup.memberof_group.getOrElse(List.empty[String]).contains(WORKGROUPS_GROUP) )
      ...
      Left(Error(Option(1),Some("Not a ***REMOVED*** workgroup"),None))
