...
import it.gov.***REMOVED***.common.sso.common.{Admin, Editor, Viewer}
...
class IntegrationService @Inject()(apiClientIPA:ApiClientIPA, supersetApiClient: SupersetApiClient, ckanApiClient: CkanApiClient, grafanaApiClient:GrafanaApiClient, registrationService: RegistrationService,kyloApiClient:KyloApiClient, impalaService:ImpalaService){
  ...
  def createDafOrganization(***REMOVED***Org:DafGroup):Future[Either[Error,Success]] = {

    logger.info("createDafOrganization")

    val groupCn = ***REMOVED***Org.groupCn
    val predefinedOrgIpaUser =     IpaUser(groupCn,
                                          ...
                                          Option(***REMOVED***Org.predefinedUserPwd),
                                          None,
                                          Option(Seq(***REMOVED***Org.groupCn)))


    val result = for {
      a0 <- stepOver( apiClientIPA.testGroupForCreation(groupCn) )
      a <- step( apiClientIPA.createGroup(Organization(***REMOVED***Org.groupCn),None) )
      ...
      c <- step( b, supersetApiClient.createDatabase(toSupersetDS(groupCn),predefinedOrgIpaUser.uid,***REMOVED***Org.predefinedUserPwd,***REMOVED***Org.supSetConnectedDbName) )
      d <- stepOver( c, kyloApiClient.createCategory(***REMOVED***Org.groupCn) )
  ...
  def createDafWorkgroup(***REMOVED***Wrk:DafGroup, orgName:String):Future[Either[Error,Success]] = {

    logger.info("createDafWorkgroup")

    val wrkName = ***REMOVED***Wrk.groupCn
    val predefinedWrkIpaUser =     IpaUser(wrkName,
      ...
      Option(***REMOVED***Wrk.predefinedUserPwd),
      None,
      Option(Seq(***REMOVED***Wrk.groupCn)))


    val result = for {
      a0 <- stepOver( apiClientIPA.testGroupForCreation(***REMOVED***Wrk.groupCn) )
      ...
      c <- step( b, supersetApiClient.createDatabase(toSupersetDS(wrkName),predefinedWrkIpaUser.uid,***REMOVED***Wrk.predefinedUserPwd,***REMOVED***Wrk.supSetConnectedDbName) )
  ...
  def createDefaultDafOrganization(passwd:String):Future[Either[Error,Success]] = {
    ...
    val ***REMOVED***Org = DafOrg(ConfigReader.defaultOrganization, passwd, "opendata")

    val groupCn = ***REMOVED***Org.groupCn
    val defaultOrgIpaUser = IpaUser( ***REMOVED***Org.groupCn,
    ...
    val result = for {
      a <- EitherT( apiClientIPA.createGroup(Organization(***REMOVED***Org.groupCn)) )
      b <- EitherT( registrationService.createDefaultUser(defaultOrgIpaUser) )
      c <- EitherT( supersetApiClient.createDatabase(toSupersetDS(groupCn),defaultOrgIpaUser.uid,***REMOVED***Org.predefinedUserPwd,***REMOVED***Org.supSetConnectedDbName) )
